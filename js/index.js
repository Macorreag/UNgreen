...
const PARQUEADEROBICI =
	"http://datosabiertos.bogota.gov.co/api/action/datastore_search?resource_id=8457f9c1-53c7-40fd-8e38-2712de094de1&q=Parqueadero%20Bici";
const CAI =
	"https://www.datos.gov.co/api/views/7pce-3uf3/rows.json?accessType=DOWNLOAD";
const BICIPARK =
...
class Parque {
	...
	constructor(id, nombre, coorCenter, localidad, coordLimit) {
		...
		this._color =
			"#111111"; /*Dejar ramdom dentro del costructor para luego usar*/
		this._parqueDibujo, (this._inventario = []);
	}
	get id() {
		return this._id;
	}
		...
		fetch(PARQUES)
			...
			.then((json) => {
				for (var i = 0; i < json.features.length; i++) {
					...
					for (
						var j = 0;
						j < json.features[i].geometry.coordinates[0][0].length;
						j++
					) {
						var point = coorGmaps(
					...
					var init = json.features[i].properties.description.indexOf(
						"NOMBRE</td>\n\n<td>"
					);
					var end = json.features[i].properties.description.indexOf(
						"</td>\n\n</tr>\n\n<tr bgcolor="
					);

					var name = json.features[i].properties.description.substring(
						end + 46,
						init + 26
					);
...
function initMap() {
	...
	map.data.loadGeoJson(REPORTESBICI);
}
